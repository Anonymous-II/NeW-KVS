...
//#define monitor_interval 100000 // monitoring module checks every interval * microseconds. hyperparameter 100000 is 0.1s
...
class DB_MASTER{
  ...
  Status Open(const Options& options, const std::string& name, const int num_instances_, const int monitor_interval_);
  ...
  void IteratorSeek(std::unique_ptr<Iterator> *single_iter_master, ReadOptions options, Iterator** iter_to_use_master, const Slice& key);

  bool IteratorValid(Iterator** iter_to_use_master);
  bool IteratorCompare(Iterator** iter_to_use_master, const Slice& key, int* idx);

  bool IteratorNext(std::unique_ptr<Iterator> *single_iter_master, ReadOptions options, Iterator** iter_to_use_master);
  void ResetFlag();
