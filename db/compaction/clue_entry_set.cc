...
std::unordered_map<std::string, std::string>::iterator Clue_Entry_Set::find_key(std::string key){
    //printf("[DEBUG] get from ce |%s|\n", key.c_str());
    std::unordered_map<std::string, std::string>::iterator it = ce_map.find(key);
    //printf("[DEBUG] get value from ce |%s|\n", it->second.c_str());
    return it;
};

void Clue_Entry_Set::overwrite(std::unordered_map<std::string, std::string>::iterator it, std::string value){
    it->second = value;
}
